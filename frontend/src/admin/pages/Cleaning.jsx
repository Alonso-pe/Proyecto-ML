...
const generateMockData = (type) => {
  const baseData = [
    ...
    { dni: '40192834', region: 'Lima', provincia: 'Lima', fecha: '2025-11-22', voto: 'Keiko Fujimori' }, 
    { dni: '123', region: 'Piura', provincia: 'Piura', fecha: '2025-11-22', voto: 'Nulo' }, 
  ];

  if (type === 'municipal') {
    return baseData.map(d => ({
      ...d,
      candidato_provincial: d.region === 'Lima' ? 'Rafael López Aliaga' : 'Rohel Sánchez',
      candidato_distrital: 'George F.',
      voto: undefined 
    }));
  }
...
export default function Cleaning() {
  ...
  useEffect(() => {
    ...
    // Cambiado a municipales_raw
    const munData = loadJSON('municipales_raw') || generateMockData('municipal');
    setPresRaw(presData);
    setMunRaw(munData);
  }, []);
  ...
  const handleClean = async (type) => {
    ...
    const raw = type === 'presidencial' ? presRaw : munRaw;
    const { clean, removed, stats } = cleanDataset(raw);
    const prefix = type === 'presidencial' ? 'presidenciales' : 'municipales';
    ...
    <motion.div className="space-y-6 w-full max-w-[1600px] mx-auto p-2 sm:p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-[#0f172a] border border-slate-800 p-1 h-auto rounded-xl w-full sm:w-auto flex sm:inline-flex">
          ...
          <TabsTrigger value="municipal" className="flex-1 sm:flex-none px-4 sm:px-6 py-2.5 text-sm data-[state=active]:bg-amber-600 data-[state=active]:text-white rounded-lg transition-all justify-center">
             Municipal
          </TabsTrigger>
        </TabsList>
        
        {['presidencial', 'municipal'].map((type) => {
          const data = type === 'presidencial' ? presRaw : munRaw;
          ...
          return (
            <TabsContent key={type} value={type} className="mt-6 space-y-6 focus-visible:outline-none">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                <QualityStat icon={Database} title="Registros" value={totalRows.toLocaleString()} subtext="Total cargado" color="blue" />
                <QualityStat icon={AlertTriangle} title="Calidad" value={`${100 - errorRate}%`} subtext={`~${estimatedErrors} errores detectados`} color={errorRate > 0 ? "amber" : "green"} />
                <QualityStat icon={cleanStats ? CheckCircle : Activity} title="Estado" value={cleanStats ? "Limpio" : "Pendiente"} subtext={cleanStats ? "Listo para uso" : "Requiere acción"} color={cleanStats ? "green" : "red"} />
              </div>

              <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                 <Card className="xl:col-span-1 bg-gradient-to-br from-slate-900 to-slate-950 border-slate-800 shadow-lg h-fit">
                    <CardHeader>
                        <CardTitle className="text-white flex items-center gap-2 text-lg"><Eraser className="w-5 h-5 text-emerald-400" /> Limpieza Automática</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2 bg-slate-800/30 p-3 rounded-lg border border-slate-700/50">
                            <div className="flex items-center gap-2 text-sm text-slate-300"><CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" /><span>Validar DNI (8 dígitos)</span></div>
                            <div className="flex items-center gap-2 text-sm text-slate-300"><CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" /><span>Eliminar duplicados</span></div>
                        </div>
                 ...
                 <Card className="xl:col-span-2 bg-[#0f172a] border-slate-800 flex flex-col overflow-hidden">
                    <div className="px-4 sm:px-6 py-4 border-b border-slate-800 bg-slate-900/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 shrink-0">
                        <h3 className="font-bold text-white text-sm flex items-center gap-2"><FileCheck className="w-4 h-4 text-blue-400" /> Vista Previa ({type === 'municipal' ? 'Municipal' : 'Nacional'})</h3>
                        <span className="text-[10px] text-slate-400 uppercase font-bold">Muestra: 100 filas</span>
                    </div>
