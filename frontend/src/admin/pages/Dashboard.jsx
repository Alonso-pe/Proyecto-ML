...
export default function Dashboard() {
  ...
  const totalMunVotes = Object.values(munVotes.byCandidate).reduce((a,b)=>a+b,0) || 1;
  const sortedMunCandidates = Object.entries(munVotes.byCandidate).sort(([,a], [,b]) => b - a);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center bg-[#0f172a] p-2 rounded-xl border border-slate-800 shadow-lg gap-4 sticky top-0 z-30 backdrop-blur-md bg-opacity-90">
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full sm:w-auto">
              <TabsList className="grid w-full grid-cols-2 bg-slate-950/50 p-1 h-10 border border-slate-800 rounded-lg">
                  ...
                  <TabsTrigger value="municipal" className="text-xs font-bold data-[state=active]:bg-amber-600 data-[state=active]:text-white transition-all rounded-md shadow-sm">MUNICIPAL</TabsTrigger>
              </TabsList>
          </Tabs>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 w-full">
        <StatCard title="Votos Emitidos" value={(activeTab === 'presidencial' ? totalPresVotes : totalMunVotes).toLocaleString()} description="Conteo Rápido" icon={Vote} colorClass={activeTab === 'presidencial' ? "text-blue-400" : "text-amber-400"} glowEffect />
        ...
        <AnimatePresence mode="wait">
            ...
            >
                <div className="lg:col-span-7 bg-[#0f172a] border border-slate-800 rounded-xl shadow-xl flex flex-col overflow-hidden h-[550px] lg:h-[680px] relative">
                    ...
                    <PeruMap data={staticData.mapData} selectedRegion={selectedRegion} onDepartmentClick={activeTab === 'municipal' ? setSelectedRegion : undefined} />
                </div>

                <div className="lg:col-span-5 flex flex-col gap-4 h-auto lg:h-[680px]">
                    <TrendChart color={activeTab === 'presidencial' ? '#3b82f6' : '#f59e0b'} label={activeTab === 'presidencial' ? 'Tendencia Presidencial' : 'Tendencia Municipal'} />
                        ...
                        <div className="px-5 py-4 border-b border-slate-800 bg-slate-900/30 backdrop-blur-sm sticky top-0 z-10 flex justify-between items-center">
                            <div>
                                <h3 className="font-bold text-white flex items-center gap-2 text-base">
                                    ...
                                    {activeTab === 'presidencial' ? "Conteo Presidencial" : (selectedRegion ? `Resultados: ${selectedRegion}` : "Resultados Provinciales")}
                                </h3>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
                            {activeTab === 'presidencial' ? (
                                sortedPresCandidates.map(([cand, val], idx) => (
                                    <CandidateRow key={cand} rank={idx + 1} label={cand} value={val} maxValue={totalPresVotes} color={idx === 0 ? "bg-emerald-500" : "bg-blue-600"} />
                                ))
                            ) : (
                                selectedRegion ? (
                                    <div className="p-2 space-y-1">
                                        {/* Datos simulados para mostrar al seleccionar región */}
                                        <CandidateRow rank={1} label="Rafael López Aliaga" value={4500} maxValue={10000} color="bg-amber-500" />
                                        <CandidateRow rank={2} label="George Forsyth" value={3200} maxValue={10000} color="bg-slate-600" />
                                        <CandidateRow rank={3} label="Daniel Urresti" value={1800} maxValue={10000} color="bg-slate-600" />
                                    </div>
                                ) : (
                                    sortedMunCandidates.map(([cand, val], idx) => (
                                        <CandidateRow key={cand} rank={idx + 1} label={cand} value={val} maxValue={totalMunVotes} color="bg-amber-600" />
                                    ))
                                )
                            )}
                            
                            {activeTab === 'municipal' && !selectedRegion && sortedMunCandidates.length === 0 && (
